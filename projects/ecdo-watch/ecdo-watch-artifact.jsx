...
import { LineChart, Line, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, ReferenceLine, Area, AreaChart, Legend } from 'recharts';
...
const StatusBanner = ({ level }) => {
  const config = {
    NOMINAL: { color: '#10b981', bg: 'rgba(16,185,129,0.1)', text: 'All parameters within historical bounds' },
    ELEVATED_DIAGNOSTIC: { color: '#f59e0b', bg: 'rgba(245,158,11,0.1)', text: 'Single-channel anomaly detected' },
    WATCH: { color: '#ef4444', bg: 'rgba(239,68,68,0.1)', text: 'Multi-channel coherent anomaly' },
  };
...
const Card = ({ step, title, status, children, span = 6 }) => {
  const statusColors = {
...
export default function ECDOWatchDashboard() {
  const kpData = useMemo(() => generateKpData(), []);
  const lodData = useMemo(() => generateLODData(), []);
  const aaData = useMemo(() => generateHistoricalAA(), []);
  const pmData = useMemo(() => generateHistoricalPM(), []);
  const magData = useMemo(() => generateMagData(), []);
  ...
  return (
    <div style={{ background: '#08080c', minHeight: '100vh', color: '#e8e8ed', fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: 20 }}>
        {/* Header */}
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, paddingBottom: 20, borderBottom: '1px solid #252532' }}>
          <div>
            <h1 style={{ fontSize: 24, fontWeight: 700, margin: 0 }}>🌍 ECDO Watch</h1>
            <p style={{ color: '#7a7a8c', fontSize: 13, margin: 0 }}>Falsification-first geophysics monitor</p>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: 16 }}>
            ...
            <div style={{ height: 180 }}>
              <ResponsiveContainer>
                <LineChart data={magData.slice(-60)}>
                  ...
                  <ReferenceLine y={-2} stroke="#f59e0b" strokeDasharray="3 3" />
                  <Line type="monotone" dataKey="bou" stroke="#4a9eff" dot={false} strokeWidth={1} opacity={0.5} />
                  <Line type="monotone" dataKey="hon" stroke="#8b5cf6" dot={false} strokeWidth={1} opacity={0.5} />
                  <Line type="monotone" dataKey="sjg" stroke="#10b981" dot={false} strokeWidth={1} opacity={0.5} />
                  <Line type="monotone" dataKey="composite" stroke="#fff" dot={false} strokeWidth={2} />
          ...
          <Card title="📊 Century-Scale Historical Context" span={12}>
            <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
              {[10, 50, 100, 150].map(y => (
                <button key={y} onClick={() => setTimeRange(y)} style={{
