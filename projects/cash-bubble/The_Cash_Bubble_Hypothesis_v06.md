...
Cash exhibits the analogous property: **one dollar is one dollar, regardless of which account it occupies.** Transfer \$100 from checking to savings, and there is \$100 less in checking and \$100 more in savings. The total is conserved. The "density" of money—its value per unit—does not change under redistribution. (This is strictly true in nominal terms. Inflation acts as atmospheric pressure on the system—present everywhere, but slow and predictable enough to factor out. The distinction is revisited in Section V.)
...
Note that all quantities in this mapping—prices, values, volumes—are denominated in nominal dollars. In engineering terms, these are *absolute* pressures: measured against a true zero rather than against some ambient reference. An inflation-adjusted framework would correspond to *gauge* pressures, where each measurement is taken relative to a time-varying atmospheric baseline. The absolute-pressure convention is adopted throughout this paper; the gauge-pressure extension is discussed in Section V.
...
**Tax, inflation, and debt**: Taxes have been ignored entirely. In reality, the work extracted from investments (realized gains) is subject to capital gains tax, reducing the net work available. A more complete model would include tax drag as thermodynamic friction or dissipation. Inflation acts as a rising atmospheric pressure on the financial system: it does not remove fluid from the container, rather it changes the external baseline against which all internal pressures are measured. The framework's nominal-dollar convention (absolute pressure) and a purchasing-power interpretation (gauge pressure) diverge as inflation accumulates. Under stable inflation the divergence is slow and predictable—the hydraulic equivalent of working at Earth's surface, where atmospheric variation is negligible. Under volatile inflation, the atmospheric reference itself becomes a dynamic variable requiring explicit treatment. This extension is deferred to future work. Debt (negative balances) introduces additional structure.
