...
export default function ApopheniaFilter() {
  ...
  const S = {
    ...
    btn: (active, color) => ({
      ...
      padding: "13px 8px",
      borderRadius: 10,
      border: `1px solid ${active ? color || C.brass : C.line}`,
      background: active ? `${(color || C.brass)}22` : "transparent",
      color: active ? color || C.brass : C.text,
      fontFamily: "'IBM Plex Mono', monospace",
      fontSize: 13,
      fontWeight: 600,
  ...
  return (
    <div style={S.app}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@500;700&family=IBM+Plex+Mono:wght@400;600&display=swap');
        ...
        button:focus-visible,input:focus-visible{outline:2px solid ${C.brass};outline-offset:2px;}`}</style>
      <div style={S.col}>
        <div style={S.eyebrow}>Instrument 001 · v{VERSION} · peirastes.com</div>
        <h1 style={S.title}>The Apophenia Filter</h1>
