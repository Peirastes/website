...
import { Plus, X, Edit2, Trash2, Calendar, ChevronDown, Download, Upload, Settings, AlertCircle, CheckCircle, LayoutGrid, List, Sparkles, Loader2, Shield, Clock, Archive, Repeat, BarChart3 } from 'lucide-react';
...
const EisenhowerTaskManager = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        const backupResult = await window.storage.get('eisenhower-backup-metadata', false);
        if (backupResult) {
          setBackupMetadata(JSON.parse(backupResult.value));
        }
      } catch (e) {
    ...
    if (!isLoading) {
      const saveData = async () => {
        try {
          ...
          await window.storage.set('eisenhower-backup-metadata', JSON.stringify(updatedMetadata), false);
        } catch (e) {
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2 mt-2 flex-wrap">
              ...
              {task.timeEstimateValue && (
                <span className="text-xs text-cyan-700 bg-cyan-100 px-2 py-0.5 rounded font-medium">
        ...
        <table className="w-full">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Estimate</th>
            ...
            {sortedTasks.map((task) => {
              ...
              return (
                <tr key={task.id} className={`hover:bg-slate-50 transition-colors ${task.percentComplete === 100 ? 'opacity-60' : ''}`}>
                  ...
                  <td className="px-4 py-3">
                    {task.timeEstimateValue ? (
                      <span className="text-xs px-2 py-1 rounded font-medium bg-cyan-100 text-cyan-700">
                        {task.timeEstimateValue}{task.timeEstimateUnit === 'hours' ? 'h' : 'd'}
                      </span>
                    ) : (
