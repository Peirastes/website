...
const EisenhowerTaskManager = () => {
  ...
  /* v4 cockpit re-architecture: the Chronosphere (bridge) is now the floor —
     the app opens onto the globe, and other views demote to pull-down charts. */
  const [view, setView] = useState('bridge');
  /* Which supplementary instrument is currently pulled OUT as a drawer over
     the globe (null = all stowed). Matrix / List / Gantt host their views. */
  const [drawerInstrument, setDrawerInstrument] = useState(null);
  /* Toggle an instrument drawer from the Command rail's Instruments menu. */
  const openInstrument = (key) => setDrawerInstrument(cur => (cur === key ? null : key));
  /* Keep the last-shown instrument mounted through the close slide so the
     drawer doesn't empty out mid-animation. Reads the live key while open,
     lingers on the previous one while it slides shut. */
  const lastDrawerRef = useRef(null);
  if (drawerInstrument) lastDrawerRef.current = drawerInstrument;
  const shownInstrument = drawerInstrument || lastDrawerRef.current;
  const DRAWER_TITLES = {
    matrix: 'Matrix', list: 'List', gantt: 'Gantt', calendar: 'Calendar',
    analytics: 'Analytics', projects: 'Projects', copilot: 'Copilot'
  };
  ...
  return (
    ...
    <div className={`etm-shell ${view === 'bridge' ? 'etm-shell--bridge' : ''} h-screen flex flex-col text-[#c8d0e0] overflow-hidden`} style={{
      ...
      <main className="flex-1 min-h-0 w-full h-full">
        ...
        {view === 'matrix' ? (
          ...
          <BridgeView
            ...
            activeInstrument={drawerInstrument} onInstrument={openInstrument}
      ...
      {/* v4: supplementary instruments as pull-out drawers over the
          Chronosphere. Only mounted on the bridge floor; the globe stays live
          underneath, dimmed by the scrim while a drawer is out. Each hosts its
          existing view verbatim (Matrix / List / Gantt). One drawer shell
          switches content; the last instrument lingers through the close slide. */}
      {view === 'bridge' && (
        <InstrumentDrawer
          open={!!drawerInstrument}
          onClose={() => setDrawerInstrument(null)}
          title={DRAWER_TITLES[shownInstrument] || 'Instrument'}
          wide={['list', 'gantt', 'calendar', 'analytics', 'projects', 'copilot'].includes(shownInstrument)}
        >
          {shownInstrument === 'matrix' && (
            <MatrixView
              tasks={liveTasks}
              getQuadrant={getQuadrant}
              sortTasks={sortTasks}
          ...
          {shownInstrument === 'analytics' && (
            <AnalyticsView
              tasks={liveTasks} calculateTaskScore={calculateTaskScore}
            />
          )}
          {shownInstrument === 'projects' && (
            <ProjectsView
              projects={projects} tasks={tasks}
              onRefresh={refreshProjects}
              setEditingTask={setEditingTask} setShowForm={setShowForm}
