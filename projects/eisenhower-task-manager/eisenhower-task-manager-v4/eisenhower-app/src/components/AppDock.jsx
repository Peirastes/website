...
/* Instruments that pull OUT as a drawer over the Chronosphere floor rather
   than hard-swapping the whole view. Only honoured when an onInstrument
   handler is wired (i.e. the Bridge's Command rail). Every view but the Bridge
   itself (the floor) hosts in the drawer shell now. */
const DRAWER_INSTRUMENTS = new Set(['matrix', 'list', 'gantt', 'calendar', 'analytics', 'projects', 'copilot']);

export const AppDock = ({
  layout = 'bar', view, setView,
  activeInstrument = null, onInstrument,
  ...
  const tabs = (
    <div className="cin-view-tabs" role="tablist">
      {VIEW_TABS.map(({ key, icon: Icon, label }) => {
        /* A drawer instrument (Matrix) toggles its drawer over the current
           floor; everything else navigates the view. */
        const isDrawer = onInstrument && DRAWER_INSTRUMENTS.has(key);
        const isActive = isDrawer ? activeInstrument === key : view === key;
        return (
          <button
            key={key}
            className={`cin-view-tab ${isActive ? 'on' : ''}`}
            onClick={() => (isDrawer ? onInstrument(key) : setView(key))}
            onPointerDown={stop}
            role="tab"
  ...
  if (layout === 'rail') {
    ...
    return (
      <>
        ...
        <div className="bridge-console__sub">◰ Instruments</div>
        {tabs}
        <span className="bridge-console__sep" />
