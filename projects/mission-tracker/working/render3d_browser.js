...
function renderEarth(ctx, cam, basis, scale, t_seconds, sun_pos, opts = {}) {
  ...
  const center_screen = project([0,0,0], cam, basis, scale);
  ...
  // Save context for clipping to the Earth disk
  ctx.save();
  ctx.beginPath();
  ctx.arc(center_screen.x, center_screen.y, r_px, 0, Math.PI*2);
  ctx.clip();

  // Background fill (deep ocean)
  ...
  // Day/night terminator: shade the night side
  // For each pixel on the visible disk, compute the surface point and check
  // its dot product with the Sun direction. Negative = night side.
  // We do this with a radial gradient approximation for performance.
  const sun_screen_x = V.dot(sun_dir, basis.right);
  const sun_screen_y = -V.dot(sun_dir, basis.up);
  const sun_depth = V.dot(sun_dir, basis.forward);

  // Simple shading: a linear gradient across the visible disk along the
  // sun-projected direction. This is an approximation but produces the
  // correct day/night appearance.
  const grad_dir_x = sun_screen_x;
  const grad_dir_y = sun_screen_y;
  const grad_mag = Math.sqrt(grad_dir_x*grad_dir_x + grad_dir_y*grad_dir_y);
  if (grad_mag > 0.01) {
    const ux = grad_dir_x / grad_mag;
    const uy = grad_dir_y / grad_mag;
    const grad = ctx.createLinearGradient(
      center_screen.x - ux*r_px, center_screen.y - uy*r_px,
      center_screen.x + ux*r_px, center_screen.y + uy*r_px
    );
...
function renderScene(ctx, scene) {
  ...
  const scale = autoFitScale(cw, ch, max_extent);
