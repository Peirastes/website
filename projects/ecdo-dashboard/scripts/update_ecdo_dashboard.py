...
# v0.7 — fixes Steps 3–5: USGS magnetometer feed + GSFC SLR C20 long-term file
#
...
def main() -> None:
    ...
    def gate_is_quiet(ts) -> bool:
        """Map any timestamp/date to its UTC day and look up quiet/disturbed."""
        d = pd.to_datetime(ts, utc=True, errors="coerce")
        if pd.isna(d):
            return False
        d = d.floor("D")
        try:
            return bool(daily_gate.reindex([d]).fillna(False).iloc[0])
        except Exception:
            return False
 if not gate.empty else pd.Series(dtype=bool)
    ...
    # ---------- Coherence diagnostics (quiet days only) ----------
    try:
        merged_for = merged_all.copy() if ("merged_all" in locals() and (merged_all is not None) and (not merged_all.empty)) else merged.copy()
        if merged_for is not None and not merged_for.empty:
            merged_for = merged_for.assign(date=pd.to_datetime(merged_for["date"], utc=True))
            merged_for["quiet"] = merged_for["date"].map(gate_is_quiet)
            q2 = merged_for[merged_for["quiet"]].dropna(subset=["mag_composite","eop_composite"]).copy()

            # Scatter
            plt.figure(figsize=(6,4))
            ax = plt.gca()
            if len(q2) >= 2:
                ax.scatter(q2["mag_composite"], q2["eop_composite"], s=18)
                c = float(q2["mag_composite"].corr(q2["eop_composite"])) if len(q2) >= 10 else float("nan")
                title = f"Quiet-day MAG vs EOP (last 90d) — N={len(q2)}" + (f", corr={c:.2f}" if not math.isnan(c) else ", corr=n/a")
            else:
                title = "Quiet-day MAG vs EOP (last 90d) — insufficient points"
            ax.set_title(title)
            ax.set_xlabel("MAG composite")
            ax.set_ylabel("EOP composite")
            write_png(assets / "coherence_scatter_90d.png")

            # Rolling 30-day trailing corr (quiet days only; requires N>=10 in window)
            days_all = pd.date_range(
                merged_for["date"].min().floor("D"),
                merged_for["date"].max().floor("D"),
                freq="D",
                tz="UTC",
            )
            corr30 = []
            for d in days_all:
                w = merged_for[(merged_for["date"] >= d - pd.Timedelta(days=30)) & (merged_for["date"] <= d)].copy()
                w = w[w["date"].map(gate_is_quiet)]
                w = w.dropna(subset=["mag_composite","eop_composite"])
                if len(w) >= 10:
                    corr30.append(float(w["mag_composite"].corr(w["eop_composite"])))
                else:
                    corr30.append(float("nan"))
            roll = pd.DataFrame({"date": days_all, "corr30": corr30})

            plt.figure(figsize=(10,4))
            ax = plt.gca()
            ax.plot(roll["date"], roll["corr30"])
            ax.set_title("Quiet-day coherence (30-day trailing corr) — requires N≥10 quiet days")
            ax.set_xlabel("UTC date")
            ax.set_ylabel("corr(MAG,EOP)")
            ax.set_ylim(-1.0, 1.0)
            write_png(assets / "coherence_rolling_30d.png")
    except Exception:
        pass
    ...
    summary_json = {
        ...
        "quiet_day_points_used_90d": n_quiet,
