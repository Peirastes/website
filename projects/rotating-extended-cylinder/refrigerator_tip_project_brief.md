...
| T4   | 5.1          | tip-recover  | 13.0        | deep excursion (just below the topple edge), recovers |
| T5   | 6.5          | topple       | 88.8        | exceeds φ_c = 22.4°, falls over                |

These five points are exactly the cases exposed as presets in the app's **Test Cases** panel (§3.5), so the documented sequence and the in-app verdicts are the same set. The **regime** is the validated quantity; `φ_max` is reference-only, since the N1 study (§5.2) found excursion depth is tens-of-percent sensitive to the contact parameters. T4 sits just below the *converged* topple edge (≈5.14 N·m): higher torques recover only at the coarse production timestep and topple under finer integration, so they are not used as recover cases.
...
**Progress.** The interactive app exists and builds (Vite + R3F, `npm run build` → static `dist/`; see README and §3.1), with the documented test cases, the lab-frame trace, and a transparent door already in place. What remains for deployment: (i) Instrument-tier styling to match the portfolio benchmarks (collision-lab / optics-lab); (ii) surfacing the basin diagram (the `sweep.mjs` result) in the UI; (iii) a responsive/mobile layout; (iv) the wrapper page, `projects.json` entry, and thumbnail that make it a listed app on the site; and (v) the undergraduate-level exposition of the regime boundaries plus a homework set that asks students to predict the boundary for a given can geometry and check it against the simulation. A "hand force × distance × duration" input mode (§6) would make the torque parameter more intuitive for students. The exposition and homework are CD/TA-hat deliverables that can follow the app; note that, per the N1 caveat, student-facing copy should treat tilt **qualitatively** (whether it topples is solid; how deep it leans is model-soft).
